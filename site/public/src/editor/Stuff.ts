class point {
  ...
  static dotP(p1: point, p2: point): number {
...
class svgCurve {
  ...
  private events: { id: string; callback: (curve: svgCurve) => void }[] =
    new Array();
  ...
  calcCenter(): point {
    ...
    if (x != x || y != y) {
      //couldn't use isNaN for some reason
  ...
  getSVGData(): string {
    return (
      ...
      this.start.x +
      " " +
      this.start.y + //start point
      " C " +
      this.startControl.x +
      " " +
      this.startControl.y + //startpoint curve towards
      " , " +
      this.startControl.x +
      " " +
      this.startControl.y + //center
      " , " +
      this.center.x +
      " " +
